...
const VideoList = ({ cachedFiles, currentIndex, onViewableItemsChanged, viewabilityConfig, navigation,selectedFilter }) => {
  ...
  const productVideosUrl = productMedia?.reduce((acc, val) => acc.concat(val), []).filter(Boolean);
  const filteredVideosUrl = productVideosUrl.filter(video =>
    selectedFilter ? video.tags.includes(selectedFilter) : true
  );


  useEffect(() => {
    // Filter videos based on selectedFilter
    if (selectedFilter === 'All') {
      setFilteredVideos(productVideosUrl);
    } else {
      const filtered = productVideosUrl.filter(video => {
        return video.tags?.some(tag => tag.toLowerCase() === selectedFilter.toLowerCase());
        // return video.tags?.includes(selectedFilter);
      });
