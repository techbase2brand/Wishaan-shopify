  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      ...
      const graphql = JSON.stringify({
        query: `query MyQuery {
          collection(id: "gid://shopify/Collection/481233142067") {
            products(first: 250) { edges { node { id title vendor handle descriptionHtml tags variants(first: 100) { edges { node { id title  inventoryQuantity price compareAtPrice selectedOptions { name value } } } } images(first: 30) { edges { node { src originalSrc altText } } } media(first: 10) { edges { node { mediaContentType ... on Video { sources { url } } } } } } } }
          }
        }`,
        variables: {}
      });
